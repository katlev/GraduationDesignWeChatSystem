// pages/teachManageStuFileLiterature/teachManageStuFileLiterature.js
...
Page({
  ...
  onLoad:async function (options) {
    ...
    if(files.data.literatureOpinion == null || files.data.literatureOpinion == ""){
      this.setData({
        literatureOpinion: null
      })
    }
  },
  // 当审核按钮发生变化时的回调
  ...
  // 提交审核
  async submitLiteratureCheck(){
    let literatureCheck = this.data.checked
    let literatureOpinion = this.data.message
    let userId = this.data.userId
    console.log(userId)

    if(literatureOpinion == ''){
      wx.showToast({
        title: '请填写审核意见',
        icon: 'none'
      })
    ...
    let submitResult = await teachRequest('/checkLiteratureFile',{check: literatureCheck,literatureOpinion: literatureOpinion,userId: userId},'GET')
